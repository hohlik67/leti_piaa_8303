...
std::vector<int> KMP(std::ifstream& input, const std::string& pattern) {   //Функция, реализующая алгоритм Кнута-Морриса-Пратта
    ...
    while (true) {
        ch = input.get();                   //Считывается очередной символ
        if (ch == EOF)
            break;
...
int main() {
    ...
    std::string pattern;
    input >> pattern;
    auto result = KMP(input, pattern);          //Запуск алгоритма Кнута-Морриса-Пратта
