...
public:
    ...
    void findStrings(string text) {
        ...
        for (int i = 0; i < text.length(); i++) {
            if (tmp->child.find(text[i]) != tmp->child.end()) {             //если можем перейти из текущей вершины, то делаем это, иначе пытаемся перейти из суффиксов
